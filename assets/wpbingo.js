...
wpbingo.HeaderSection = (function () {
	...
	function Header(container) {
		...
		this.cache.$menuMobileChildToggle.on('click', function (evt) {
			evt.preventDefault();
			var $this = $(this);
			var $navItem = $this.closest('.menu-mobile__nav-item');
			var $submenu = $navItem.children('.mm-nav__links');

			if ($this.hasClass('mm-nav__prev')) {
				// Back button clicked - hide submenu
				var $parentSubmenu = $this.closest('.mm-nav__links');
				var $parentNavItem = $parentSubmenu.closest('.menu-mobile__nav-item');
				$parentSubmenu.removeClass('show');
				$parentNavItem.removeClass('active');
			} else {
				// Toggle submenu
...
// Custom Tab Handler for Product Tabs
$(document).ready(function () {
	$('body').on('click', '[data-toggle="tab"]', function (e) {
		...
		if ($targetPane.length) {
			$targetPane.closest('.tab-content').find('.tab-pane.active').removeClass('active show');
			$targetPane.addClass('active show');

			// Re-initialize Swiper if needed
			var $swiper = $targetPane.find('.bwp-swiper-slider');
			if ($swiper.length && !$swiper.hasClass('swiper-initialized')) {
				wpbingo.load_swiper_carousel($swiper);
			} else if ($swiper.length && $swiper[0].swiper) {
